...
const BookingInfo = ({ bookings }) => {
  return (
    <div className="max-w-7xl mx-auto lg:w-[50%]">
      <div className="flex flex-col gap-5 md:flex-row justify-between items-center">
        ...
        <div>
          <Link href={"/all-appointment"}>
            <Button
              className={
                "bg-[#0a9396] text-white font-bold rounded-lg w-[100%]"
              }
            >
              + New Appointment
            </Button>
      ...
      {bookings.length === 0 ? (
        <div className="border-y-4 rounded-xl shadow-2xl border-[#0a9396] text-center p-10">
        ...
        <div className="space-y-10">
          {bookings.map((booking) => (
            <div
              className="border-l-8 shadow-2xl p-3 rounded-lg border-l-black"
              key={booking._id}
            >
              <div className="flex flex-col md:flex-row md:items-center gap-5">
                <div className="border-t-2 border-l-5 border-[#0a9396] rounded-lg p-3 w-full">
                  <h2 className="text-xl text-bold text-center mb-5 border-b-2">
                ...
                <div className="border-r-5 border-b-2 border-[#0a9396] rounded-lg p-3 w-full">
                  <div>
                    <h1 className="text-xl text-bold text-center mb-5 border-b-2">
                      Patient's Contact Method
                    </h1>
